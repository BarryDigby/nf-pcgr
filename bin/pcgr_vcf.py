...
def pcgr_ready_vcf(sample):
    ...
    avg_df = pd.concat(list(vcf_dict.values())).assign(QUAL = lambda d: d['QUAL'].apply(lambda s: 0 if s == "." else float(s) )).groupby(level=[0,1,2,3]).agg({'TAF':'max', 'TDP':'max', 'NAF':'max', 'NDP':"max", 'QUAL':"max", })
    ...
    # convert TAL to AL
    algo_dict = {'1':'mutect2', '2':'freebayes', '3':'strelka',
    ...
    avg_df = avg_df.drop(['NDP', 'NAF', 'TDP', 'TAF', 'TAL', 'ADT', 'ADN'], axis=1) # drop ADT ADN without using in INFO

    # ADd FORMAT column
    avg_df['FORMAT'] = avg_df[['FMT_GT', 'FMT_DPN', 'FMT_DPT', 'FMT_ADN', 'FMT_ADT', 'FMT_AL']].apply(lambda x: ':'.join(x[x.notnull()]), axis = 1)
    avg_df = avg_df.drop(['FMT_GT', 'FMT_DPN', 'FMT_DPT', 'FMT_ADN', 'FMT_ADT', 'FMT_AL'], axis=1)

    # Add sample column
    avg_df[f'{sample_id}'] = avg_df[['SAMPLE_GT', 'SAMPLE_DPN', 'SAMPLE_DPT', 'SAMPLE_ADN', 'SAMPLE_ADT', 'SAMPLE_AL']].apply(lambda x: ':'.join(x[x.notnull()]), axis = 1)
