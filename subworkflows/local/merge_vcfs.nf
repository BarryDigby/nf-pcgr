...
workflow MERGE_VCFS {
    ...
    emit:
    pcgr_ready_vcf = params.cna_analysis ? PCGR_VCF.out.vcf.join( files.map{ it -> meta = it[2].simpleName; return [ meta, it[3] ]} ) : PCGR_VCF.out.vcf.map{ meta, vcf, tbi -> return [ meta, vcf, tbi, [] ] }
    //sample_vcfs
}
